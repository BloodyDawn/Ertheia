...
{
	private static final Logger _log = LogManager.getLogger(OlympiadGameManager.class);
	...
	public int getCountOfStadiums()
	{
		return _tasks.length;
	}

	public void notifyCompetitorDamage(L2PcInstance player, int damage)
	{
		if(player == null)
		{
			return;
		...
		int id = player.getOlympiadController().getGameId();
		if(id < 0 || id >= _tasks.length)
		{
			return;
		}

		AbstractOlympiadGame game = _tasks[id].getGame();
		if(game != null)
		{
