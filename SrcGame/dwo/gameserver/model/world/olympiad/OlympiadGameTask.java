...
public class OlympiadGameTask implements Runnable
{
	...
	protected static final Logger _log = LogManager.getLogger(OlympiadGameTask.class);
	...
	public void attachGame(AbstractOlympiadGame game)
	{
		...
		{
			_log.log(Level.WARN, "Attempt to overwrite non-finished game in state " + _state);
			return;
		}
	...
	public void run()
	{
		try
		{
			...
			switch(_state)
			{
				...
				case BATTLE_IN_PROGRESS:
					_countDown += 1000;
					if(checkBattle() || _countDown > BATTLE_PERIOD)
					{
						_state = GameState.GAME_STOPPED;
					}
					break;
		...
		catch(Exception e)
		{
			switch(_state)
			{
				...
				case CLEANUP:
				case IDLE:
					_log.log(Level.WARN, "Unable to return players back in town, exception: " + e.getMessage());
					_state = GameState.IDLE;
					_game = null;
					return;
			}

			_log.log(Level.WARN, "Exception in " + _state + ", trying to port players back: " + e.getMessage(), e);
			_state = GameState.GAME_STOPPED;
	 ...
	 * @return {@code true} if no participants defaulted.
	 */
	private boolean startGame()
	{
		try
		{
			...
			_zone.closeDoors();
			_zone.showBuffers();
            _zone.getPlayersInside().forEach(p ->
            {
                p.setPunishLevel(1);
                p.sendMessage("Вам запрещено использовать чат во время боя!");
            }); //блочим чат в соответствии с новым апдейтом хроник
	 ...
	 * @return {@code true} if all participants online and ready on the stadium.
	 */
	...
	private void stopGame()
	{
		try
		{
            ...
            _zone.getPlayersInside().forEach(p ->
                    p.setPunishLevel(0)); //снимаем блок чата(альтернативный вариант с БЧ админа
		}
		catch(Exception e)
		{
			_log.log(Level.ERROR, e.getMessage(), e);
