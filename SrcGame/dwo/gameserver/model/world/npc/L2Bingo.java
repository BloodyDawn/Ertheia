...
public class L2Bingo
{
	...
	protected String getBoard()
	{
		...
		for(int i = 1; i <= 9; ++i)
		{
			String cell = new StringBuilder().append("%cell").append(i).append('%').toString();
			int num = board.get(i - 1);
			result = guesses.contains(Integer.valueOf(num)) ? result.replaceFirst(cell, new StringBuilder().append("<font color=\"").append(guesses.size() == 6 ? "ff0000" : "ffff00").append("\">").append(num).append("</font>").toString()) : result.replaceFirst(cell, "?");
		}
		return result;
	}
	...
	public String getDialog(String _msg)
	{
		...
		result = guesses.isEmpty() ? result.replaceFirst("%msg%", "I've arranged 9 numbers on the panel.<br>Now, select your %choicenum% number.") : result.replaceFirst("%msg%", _msg.equalsIgnoreCase("") ? "Now, choose your %choicenum% number." : _msg);
