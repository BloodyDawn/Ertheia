...
import dwo.gameserver.model.world.npc.L2PetData.L2PetSkillLearn;
...
public class L2BabyPetInstance extends L2PetInstance
{
	...
	private FastList<SkillHolder> _buffs;
	private SkillHolder _majorHeal;
	private SkillHolder _minorHeal;
	private SkillHolder _recharge;
	...
	public void onSpawn()
	{
		...
		for(L2PetSkillLearn psl : PetDataTable.getInstance().getPetData(getNpcId()).getAvailableSkills())
		{
			...
			if(skill != null)
			{
				if(skill.getTargetType() == L2TargetType.TARGET_SELF)
				{
					continue;
				}
	...
	protected boolean unSummon(L2PcInstance owner, boolean ignoreDeathAndVis)
	{
		synchronized(this)
		{
			stopCastTask();
			abortCast();
			return super.unSummon(owner, false);
		}
	}
	...
	protected void castSkill(L2Skill skill)
	{
		...
		boolean previousFollowStatus = getFollowStatus();
		...
		sendPacket(SystemMessage.getSystemMessage(SystemMessageId.PET_USES_S1).addSkillName(skill));
	...
	private class CastTask implements Runnable
	{
		...
		public void run()
		{
			...
			if(owner != null && !owner.isDead() && !owner.isInvul() && !_baby.isCastingNow() && !_baby.isBetrayed() && !_baby.isMuted() && !_baby.isOutOfControl() && _bufferMode && _baby.getAI().getIntention() != CtrlIntention.AI_INTENTION_CAST)
			{
				...
				if(_majorHeal != null && owner.isInCombat()) // pet heal only if owner in combat stance
				{
					/**
					...
					boolean isImprovedBaby = PetDataTable.isImprovedBaby(getNpcId());
					if(isImprovedBaby && hpPercent < 0.3 || !isImprovedBaby && hpPercent < 0.15)
					{
						skill = _majorHeal.getSkill();
						if(!_baby.isSkillDisabled(skill) && Rnd.getChance(75))
						{
					...
					else if(!_majorHeal.getSkill().equals(_minorHeal.getSkill()) && (isImprovedBaby && hpPercent < 0.7 || !isImprovedBaby && hpPercent < 0.8))
					{
						//Cast _minorHeal only if it's different than _majorHeal, then pet has two heals available.
						skill = _minorHeal.getSkill();
						if(!_baby.isSkillDisabled(skill) && Rnd.getChance(25))
						{
							if(_baby.getCurrentMp() >= skill.getMpConsume())
							{
								castSkill(skill);
						...
						for(L2Effect e : effects)
						{
							...
							while(iter.hasNext())
							{
								...
								else
								{
									...
									if(skill.hasEffects() && !"none".equals(skill._effectTemplates[0].getAbnormalType()) && e.getAbnormalType().equals(skill._effectTemplates[0].getAbnormalType()) && e.getAbnormalLvl() >= skill._effectTemplates[0].getAbnormalLevel())
									{
										iter.remove();
									}
									String[] eLockedStackTypes = e.getEffectTemplate().getAbnormalLockedStackTypes();
									if(eLockedStackTypes != null)
									{
										for(String eLockedStackType : eLockedStackTypes)
										{
											if(eLockedStackType.equals(skill._effectTemplates[0].getAbnormalType()))
											{
												iter.remove();
											}
										}
				...
				if(_recharge != null && owner.isInCombat() // recharge casted only if owner in combat stance
					&& owner.getCurrentMp() / owner.getMaxMp() < 0.6 && Rnd.getChance(60))
				{
