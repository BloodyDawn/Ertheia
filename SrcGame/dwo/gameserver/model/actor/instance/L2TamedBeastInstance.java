...
public class L2TamedBeastInstance extends L2FeedableBeastInstance
{
	...
	private List<L2Skill> _beastSkills;
	...
	public L2TamedBeastInstance(int npcTemplateId, L2PcInstance owner, int foodSkillId, int x, int y, int z)
	{
		...
		setHome(x, y, z);
		getLocationController().spawn(x, y, z);
	}
	...
	public void setOwner(L2PcInstance owner)
	{
		...
		{
			getLocationController().delete();    // despawn if no owner
		}
	}
	...
	public void onOwnerGotAttacked(L2Character attacker)
	{
		...
		if(HPRatio >= 0.8)
		{
			...
			getTemplate().getSkills().values().stream().filter(skill -> skill.getSkillType() == L2SkillType.DEBUFF && Rnd.get(3) < 1 && attacker != null && attacker.getFirstEffect(skill) != null).forEach(skill -> sitCastAndFollow(skill, attacker));
		}
	 ...
	 *
	 * @see L2Character#doCast(L2Skill)
	 */
	...
	private static class CheckDuration implements Runnable
	{
		...
		public void run()
		{
			...
			if(_tamedBeast._isFreyaBeast)
			{
				...
				{
					owner.destroyItem(ProcessType.NPC, item, 1, _tamedBeast, true);
					_tamedBeast.broadcastPacket(new SocialAction(_tamedBeast.getObjectId(), 3));
				}
				else
				{
					_tamedBeast.getLocationController().delete();
				}
			}
			else
			{
	...
	private class CheckOwnerBuffs implements Runnable
	{
		...
		public void run()
		{
			...
			if((_numBuffs << 1) / 3 > totalBuffsOnOwner)
			{
