...
import dwo.scripts.quests._00620_FourGoblets;
...
public class L2SepulcherMonsterInstance extends L2MonsterInstance
{
	...
	private void giveCup(L2Character killer)
	{
		...
		if(player.getParty() != null)
		{
			...
			{
				QuestState qs = mem.getQuestState(_00620_FourGoblets.class);
				if(qs != null && (qs.isStarted() || qs.isCompleted()) && mem.getInventory().getItemByItemId(oldBrooch) == null)
				{
					mem.addItem(ProcessType.QUEST, cupId, 1, mem, true);
				}
			}
		}
		else
		{
			QuestState qs = player.getQuestState(_00620_FourGoblets.class);
			if(qs != null && (qs.isStarted() || qs.isCompleted()) && player.getInventory().getItemByItemId(oldBrooch) == null)
			{
				player.addItem(ProcessType.QUEST, cupId, 1, player, true);
			}
		}
	}
	...
	private static class OnDeadEvent implements Runnable
	{
		...
		public void run()
		{
			switch(_activeChar.getNpcId())
			{
				...
				case 18147:
				case 18148:
					FourSepulchersManager.getInstance().spawnMonsterForTriggerBox(_activeChar.mysteriousBoxId);
					break;
	...
	private class VictimShout implements Runnable
	{
		...
		public void run()
		{
			...
			broadcastPacket(new NS(getObjectId(), ChatType.ALL, getNpcId(), "forgive me!!"));
		}
	}
	...
	private class VictimSpawnKeyBox implements Runnable
	{
		...
		public void run()
		{
			...
			FourSepulchersManager.getInstance().spawnKeyBox(_activeChar);
			broadcastPacket(new NS(getObjectId(), ChatType.ALL, getNpcId(), "Many thanks for rescue me."));
			if(_victimShout != null)
			{
				_victimShout.cancel(true);
			}
