...
public class L2FortCommanderInstance extends L2DefenderInstance
{
	 ...
	 * @return {@code true} if a siege is in progress and the L2Character attacker isn't a Defender.
	 */
	@Override
	public boolean isAutoAttackable(L2Character attacker)
	{
		...
		boolean isFort = getFort() != null && getFort().getFortId() > 0 && getFort().getSiege().isInProgress() && !getFort().getSiege().checkIsDefender(((L2PcInstance) attacker).getClan());
		...
		if(spawn != null && canTalk())
		{
			...
			for(SiegeSpawn spawn2 : commanders)
			{
				if(spawn2.getNpcId() == spawn.getNpcId())
				{
					...
					if(npcString != null)
					{
						broadcastPacket(new NS(getObjectId(), ChatType.SHOUT, getNpcId(), npcString));
						_canTalk = false;
