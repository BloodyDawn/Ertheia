...
public class SummonStatus extends PlayableStatus
{
	...
	public void reduceHp(double value, L2Character attacker, boolean awake, boolean isDOT, boolean isHPConsumption)
	{
		...
		if(getActiveChar().getOwner().getParty() != null)
		{
			...
			if(caster != null && getActiveChar().getParty() != null && Util.checkIfInRange(1000, getActiveChar(), caster, true) && !caster.isDead() && !getActiveChar().getOwner().equals(caster) && getActiveChar().getParty().getMembers().contains(caster))
			{
				int transferDmg = 0;
				...
				if(transferDmg > 0 && attacker instanceof L2Playable)
				{
					...
					{
						if(Util.checkIfInRange(1000, member, caster, false) && !member.equals(caster))
						{
							membersInRange++;
						}
					}
