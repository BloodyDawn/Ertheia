...
public class PetStat extends SummonStat
{
	...
	public boolean addExpAndSp(long addToExp, int addToSp)
	{
		...
		getActiveChar().updateAndBroadcastStatus(1);
		getActiveChar().sendPacket(SystemMessage.getSystemMessage(SystemMessageId.PET_EARNED_S1_EXP).addNumber((int) addToExp));
		return true;
	}
	...
	public boolean addLevel(byte value)
	{
		...
		// Send a ServerMode->Client packet PetInfo to the L2PcInstance
		getActiveChar().updateAndBroadcastStatus(1);
	...
	public long getExpForLevel(int level)
	{
		...
		catch(NullPointerException e)
		{
			...
			{
				_log.log(Level.ERROR, "Pet objectId:" + getActiveChar().getObjectId() + ", NpcId:" + getActiveChar().getNpcId() + ", level:" + level + " is missing data from pets_stats table!");
			}
			throw e;
		}
	}
