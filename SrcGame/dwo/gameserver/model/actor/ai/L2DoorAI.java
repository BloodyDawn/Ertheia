...
public class L2DoorAI extends L2CharacterAI
{
	...
	private class onEventAttackedDoorTask implements Runnable
	{
		...
		public void run()
		{
			_door.getKnownSiegeGuards().stream().filter(guard -> _actor.isInsideRadius(guard, guard.getFactionRange(), false, true) && Math.abs(_attacker.getZ() - guard.getZ()) < 200).forEach(guard -> guard.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, _attacker, 15));
		}
	}
}
