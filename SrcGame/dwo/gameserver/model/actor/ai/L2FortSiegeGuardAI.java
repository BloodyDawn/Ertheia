...
public class L2FortSiegeGuardAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if(target == null || target instanceof L2DefenderInstance || target.isNpc() || target.isDoor() || target.isAlikeDead() || target.isPlayable())
		{
		...
		synchronized(this)
		{
			if(intention == AI_INTENTION_IDLE /*|| intention == AI_INTENTION_ACTIVE*/) // active becomes idle if only a summon is present
			{
				...
				if(!_actor.isAlikeDead())
				{
					...
					intention = !npc.getKnownList().getKnownPlayers().isEmpty() ? AI_INTENTION_ACTIVE : AI_INTENTION_IDLE;
				}
	...
	private void thinkActive()
	{
		...
		if(_globalAggro >= 0)
		{
			...
			L2Character hated;
			hated = _actor.isConfused() ? getAttackTarget() : npc.getMostHated();
		...
		if(_actor.getWalkSpeed() >= 0)
		{
			...
			else if(_actor instanceof L2DefenderInstance)
			{
		...
		for(L2Character cha : _actor.getKnownList().getKnownCharactersInRadius(1000))
		{
			...
			if(npc.getAI() != null) // TODO: possibly check not needed
			{
				if(!npc.isDead() && Math.abs(target.getZ() - npc.getZ()) < 600
					...
					&& (npc.getAI()._intention == AI_INTENTION_IDLE || npc.getAI()._intention == AI_INTENTION_ACTIVE)
	...
	private void attackPrepare()
	{
		...
		Collection<L2Skill> skills;
		double dist_2;
		int range;
		L2DefenderInstance sGuard;
		sGuard = _actor instanceof L2FortCommanderInstance ? (L2FortCommanderInstance) _actor : (L2DefenderInstance) _actor;
		L2Character attackTarget = getAttackTarget();
		...
		try
		{
			...
			range = _actor.getPhysicalAttackRange() + _actor.getTemplate().getCollisionRadius(_actor) + attackTarget.getTemplate().getCollisionRadius(attackTarget);
		...
		{
			//_log.log(Level.WARN, "AttackableAI: Attack target is NULL.");
		...
		{
			// CastleSiegeEngine guards differ from normal mobs currently:
			...
			for(L2Skill sk : skills)
			{
				...
				if(dist_2 <= castRange * castRange && castRange > 70 && !_actor.isSkillDisabled(sk) && _actor.getCurrentMp() >= _actor.getStat().getMpConsume(sk) && !sk.isPassive())
				{
					...
					if(sk.getSkillType() == L2SkillType.BUFF || sk.getSkillType() == L2SkillType.HEAL)
					{
						...
						else if(sk.getSkillType() == L2SkillType.BUFF)
						{
		...
		else if(_actor.isMuted() && dist_2 > range * range) // Else, if the actor is muted and far from target, just "move to pawn"
		{
			// Temporary hack for preventing guards jumping off towers,
			// before replacing this with effective geodata checks and AI modification
			double dz = _actor.getZ() - attackTarget.getZ();
		...
		else if(dist_2 <= range * range)
		{
			...
			L2Character hated;
			hated = _actor.isConfused() ? attackTarget : ((L2Attackable) _actor).getMostHated();
			...
			if(!_actor.isMuted() && Rnd.getChance(5))
			{
				for(L2Skill sk : skills)
				{
					int castRange = sk.getCastRange();
