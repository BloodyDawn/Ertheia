...
public class L2SiegeGuardAI extends L2CharacterAI implements Runnable
{
	...
	{
		// Проверяем, чтобы цель была игроком или петом
		if(target == null || !target.isPlayable() || target.isAlikeDead())
		{
			return false;
		}
	...
	void changeIntention(CtrlIntention intention, Object arg0, Object arg1)
	{
		synchronized(this)
		{
			if(Config.DEBUG)
			{
				_log.log(Level.DEBUG, "L2SiegeAI.changeIntention(" + intention + ", " + arg0 + ", " + arg1 + ')');
			}
	...
	private void thinkAttack()
	{
		if(Config.DEBUG)
		{
			_log.log(Level.DEBUG, "L2SiegeGuardAI.thinkAttack(); timeout=" + (_attackTimeout - GameTimeController.getInstance().getGameTicks()));
		}
