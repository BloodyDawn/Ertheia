...
public class L2EtcItem extends L2Item
{
	...
	public L2EtcItem(StatsSet set)
	{
		...
		if(capsuled_items != null)
		{
			...
			for(String part : split)
			{
				...
				if(data.length != 4)
				{
					_log.log(Level.ERROR, StringUtil.concat("> Couldnt parse ", part, " in capsuled_items! item ", toString()));
					continue;
				}
				...
				if(max < min)
				{
					_log.log(Level.INFO, StringUtil.concat("> Max amount < Min amount in ", part, ", item ", toString()));
					continue;
				}
			...
			//check for handler
			if(_handler == null)
			//_log.log(Level.WARN, "Item "+this+ " define capsuled_items but missing handler.");
			{
	...
	{
		return _type == L2EtcItemType.SHOT || _type == L2EtcItemType.POTION; // || (type == L2EtcItemType.SCROLL));
	}
	...
	public boolean isCharSpiritshot()
	{
		return getDefaultAction() == L2ActionType.spiritshot;
	}

	public boolean isSummonSpiritshot()
	{
		return getDefaultAction() == L2ActionType.summon_spiritshot || getDefaultAction() == L2ActionType.summon_blessed_spiritshot;
	}

	public boolean isCharSoulshot()
	{
		return getDefaultAction() == L2ActionType.soulshot;
	}

	public boolean isSummonSoulshot()
	{
		return getDefaultAction() == L2ActionType.summon_soulshot || getDefaultAction() == L2ActionType.summon_blessed_soulshot;
	}
