...
public class PcInventory extends Inventory
{
	...
	public L2ItemInstance[] getAvailableItems(boolean allowAdena, boolean allowNonTradeable, boolean feightable)
	{
		...
		for(L2ItemInstance item : _items)
		{
			...
			else if(feightable && item.getItemLocation() == ItemLocation.INVENTORY && item.isFreightable() || item.isAvailable(_owner, allowAdena, allowNonTradeable) && canManipulateWithItemId(item.getItemId()))
			{
				list.add(item);
			}
		}
	...
	{
		FastList<L2ItemInstance> list = FastList.newInstance();
		list.addAll(_items.stream().filter(item -> item != null && item.isAugmented()).collect(Collectors.toList()));
	...
	{
		FastList<L2ItemInstance> list = FastList.newInstance();
		list.addAll(_items.stream().filter(item -> item != null && item.getElementals() != null).collect(Collectors.toList()));
	...
	public void addItem(ProcessType process, int itemId, long count, L2PcInstance actor, Object reference, boolean sendMessage)
	{
	...
	{
		return !(_blockMode == 0 && ArrayUtils.contains(_blockItems, itemId) || _blockMode == 1 && !ArrayUtils.contains(_blockItems, itemId));
	}
