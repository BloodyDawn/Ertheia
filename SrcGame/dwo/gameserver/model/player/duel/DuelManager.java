...
{
	private static final Logger _log = LogManager.getLogger(DuelManager.class);
	// =========================================================
	// Data Field
	...
	private DuelManager()
	{
		_log.log(Level.INFO, "DuelManager: Initializing.");
		_duels = new FastList<>();
	}
	...
	{
		for(FastList.Node<Duel> e = _duels.head(), end = _duels.tail(); !(e = e.getNext()).equals(end); )
		{
	...
	public void addDuel(L2PcInstance playerA, L2PcInstance playerB, int partyDuel)
	{
		...
		if(partyDuel == 1)
		{
			...
			{
				if(temp.getPvPFlagController().isFlagged())
				{
					playerInPvP = true;
					break;
				}
		...
		{
			if(playerA.getPvPFlagController().isFlagged() || playerB.getPvPFlagController().isFlagged())
			{
	...
	public void broadcastToOppositTeam(L2PcInstance player, L2GameServerPacket packet)
	{
		...
		if(duel.getPlayerA().equals(player))
		{
			duel.broadcastToTeam2(packet);
		}
		else if(duel.getPlayerB().equals(player))
		{
