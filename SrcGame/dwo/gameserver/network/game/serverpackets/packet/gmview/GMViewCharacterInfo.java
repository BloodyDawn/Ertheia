...
public class GMViewCharacterInfo extends L2GameServerPacket
{
	...
	protected void writeImpl()
	{
		...
		writeF(ExperienceTable.getInstance().getExpPercent(_activeChar.getLevel(), _activeChar.getExp()));
		...
		writeD(_activeChar.getPvPFlagController().getStateValue()); // 0-non-pvp  1-pvp = violett name
		...
		writeH(_activeChar.getRecommendations()); //Blue value for name (0 = white, 255 = pure blue)
		writeD(_activeChar.getClassId().getId());
		...
		writeC(_activeChar.getOlympiadController().isHero() ? 0x01 : 0x00);
