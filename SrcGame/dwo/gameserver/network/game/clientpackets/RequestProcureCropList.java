...
public class RequestProcureCropList extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		int castleId = ((L2Npc) manager).getCastle().getCastleId();
		...
		for(Crop i : _items)
		{
			...
			if(fee > 0 && !player.reduceAdena(ProcessType.MANOR, fee, manager, true))
			{
				continue;
			}

			if(!player.destroyItem(ProcessType.MANOR, i.getObjectId(), i.getCount(), manager, true))
			{
				continue;
			}

			player.addItem(ProcessType.MANOR, i.getReward(), rewardItemCount, manager, true);
		}
	}
