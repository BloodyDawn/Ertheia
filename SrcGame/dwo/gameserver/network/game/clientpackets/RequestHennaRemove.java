...
public class RequestHennaRemove extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(!getClient().getFloodProtectors().getTransaction().tryPerformAction(FloodAction.HENNA_REMOVE))
		{
			return;
		}
		...
		for(int i = 1; i <= 3; i++)
		{
			...
			{
				if(activeChar.getAdenaCount() >= henna.getPrice() / 5)
				{
	...
	{
		return "[C] BC RequestHennaRemove";
	}
}