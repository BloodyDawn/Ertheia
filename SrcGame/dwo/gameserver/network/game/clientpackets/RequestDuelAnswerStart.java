...
public class RequestDuelAnswerStart extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(_response == 1)
		{
			...
			{
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.C1_CANNOT_DUEL_BECAUSE_C1_IS_ALREADY_ENGAGED_IN_A_DUEL).addString(requestor.getName()));
				return;
			}
			if(player.isInDuel())
			{
		...
		{
			requestor.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.C1_IS_SET_TO_REFUSE_DUEL_REQUEST).addPcName(player));
		}
		else
		{
