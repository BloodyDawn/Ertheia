...
import dwo.gameserver.network.game.serverpackets.packet.ex.ExDuelAskStart;
...
public class RequestDuelStart extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		L2PcInstance targetChar = WorldManager.getInstance().getPlayer(_player);
		if(activeChar == null)
		{
			return;
		}
		...
		if(activeChar.equals(targetChar))
		{
			activeChar.sendPacket(SystemMessageId.THERE_IS_NO_OPPONENT_TO_RECEIVE_YOUR_CHALLENGE_FOR_A_DUEL);
			return;
		}
		...
		{
			activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.C1_CANNOT_RECEIVE_A_DUEL_CHALLENGE_BECAUSE_C1_IS_TOO_FAR_AWAY).addString(targetChar.getName()));
			return;
		}
		...
		if(_partyDuel == 1)
		{
			...
			if(activeChar.getParty().getMembers().contains(targetChar))
			{
		...
		else // 1vs1 duel
		{
			...
			{
				activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.C1_IS_BUSY_TRY_LATER).addString(targetChar.getName()));
			}
			else
			{
				...
				if(Config.DEBUG)
				{
					_log.log(Level.DEBUG, activeChar.getName() + " requested a duel with " + targetChar.getName());
				}

				activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.C1_HAS_BEEN_CHALLENGED_TO_A_DUEL).addString(targetChar.getName()));
				targetChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.C1_HAS_CHALLENGED_YOU_TO_A_DUEL).addString(activeChar.getName()));
			}
		}
	}
