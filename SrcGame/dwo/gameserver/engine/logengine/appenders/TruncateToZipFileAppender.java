package dwo.gameserver.engine.logengine.appenders;
...
public class TruncateToZipFileAppender extends FileAppender
{
	...
	private String[] ignoreList = {
		"donate", "vote", "lameguard", "olympiad", "chat", "damage_pdam", "damage_mdam", "audit", "item",
		"enchant_item", "enchant_skill"
	};
	private String[] ignoreList2 = {"errors", "debug"};
	 ...
	 * Truncation is done by {@link #truncate(File)}
	 *
	...
	protected void truncate(File file)
	{
		...
		// Если размер лога консоли <~ 100Мб, то не зипуем
		//if (file.exists() && file.length() < sizeIgnore && Util.StringEquals(file.getName(), ignoreList))
		if(file.length() < 100 * 1024 * 1024 && Util.StringEquals(file.getName().split("\\.")[0], ignoreList))
		{
			return;
		}
		if(file.length() < 5 * 1024 * 1024 && Util.StringEquals(file.getName().split("\\.")[0], ignoreList2))
		{
			return;
		}
		...
		// Если размер лога консоли <~ 60Кб, то не зипуем
		if(file.length() >= 0 && file.length() < 90000 && (file.getName().contains("console") || file.getName().contains("debug")))
		{
			LogLog.debug("Compression of file: Deleting trash log " + file.getAbsolutePath());
			if(!file.delete())
			{
		...
		if(FileUtils.isFileOlder(file, ManagementFactory.getRuntimeMXBean().getStartTime()))
		{
			...
			{
				throw new Error("Invalid date formate for backup files: " + backupDateFormat, e);
			}
			...
			finally
			{
				if(zos != null)
				{
					...
					{
						// not critical error
						LogLog.error("Can't close zip file", e);
					}
				}
				...
				if(fis != null)
				{
					...
					{
						LogLog.error("Can't close zipped file", e);
					}
				}
			}
	...
	/**
	 * Sets date format for backup files represented as string
	 *
