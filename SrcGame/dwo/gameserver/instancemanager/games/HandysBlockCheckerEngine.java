 ...
 * @author BiggBoss
 */
public class HandysBlockCheckerEngine
{
	private static final Logger _log = LogManager.getLogger(HandysBlockCheckerEngine.class);
	...
	private boolean _abnormalEnd;

	public HandysBlockCheckerEngine(ArenaParticipantsHolder holder, int arena)
	{
		_holder = holder;
		if(arena > -1 && arena < 4)
		{
	...
	/**
	 * @return the current red team points
	 */
	...
	/**
	 * @return the current blue team points
	 */
	...
	public int getPlayerPoints(L2PcInstance player, boolean isRed)
	{
		...
		return isRed ? _redTeamPoints.get(player) : _blueTeamPoints.get(player);
	}
	...
	public void endEventAbnormally()
	{
		try
		{
			synchronized(this)
			{
				...
				if(Config.DEBUG)
				{
					_log.log(Level.DEBUG, "Handys Block Checker Event at arena " + _arena + " ended due lack of players!");
				}
			}
		}
		catch(Exception e)
		{
			_log.log(Level.ERROR, "Couldnt end Block Checker event at " + _arena, e);
		}
	}
	...
	public class StartEvent implements Runnable
	{
		...
		private void setUpPlayers()
		{
			...
			ExBlockUpSetState initialPoints = new ExBlockUpSetState(300, _bluePoints, _redPoints);
			...
			for(L2PcInstance player : _holder.getAllPlayers())
			{
				...
				int tc = _holder.getPlayerTeam(player) << 1;
				...
				player.getEventController().setHandysBlockCheckerEventArena((byte) _arena);
		...
		public void run()
		{
			...
			if(_arena == -1)
			{
				_log.log(Level.ERROR, "Couldnt set up the arena Id for the Block Checker event, cancelling event...");
				return;
			}
			_isStarted = true;
			// Spawn the blocks
		...
		public void run()
		{
			...
			if(_round == 1 || _round == 2)
			{
				...
				try
				{
					...
					SpawnTable.getInstance().addNewSpawn(girlSpawn);
					girlSpawn.init();
				...
				{
					_log.log(Level.ERROR, "Couldnt Spawn Block Checker NPCs! Wrong instance type at npc table?", e);
				}
			}
	...
	private class CarryingGirlUnspawn implements Runnable
	{
		...
		public void run()
		{
			if(_spawn == null)
			{
				_log.log(Level.WARN, "HBCE: Block Carrying Girl is null");
				return;
			}
			SpawnTable.getInstance().deleteSpawn(_spawn);
			_spawn.stopRespawn();
			_spawn.getLastSpawn().getLocationController().delete();
		}
	}
	...
	private class EndEvent implements Runnable
	{
		...
		private void rewardAsWinner(boolean isRed)
		{
			...
			for(L2PcInstance pc : tempPoints.keySet())
			{
				...
				{
					pc.addItem(ProcessType.EVENT, 13067, 2, pc, true);
				}
				else
				{
			...
			if(winner1 != null)
			{
				winner1.addItem(ProcessType.EVENT, 13067, 8, winner1, true);
			}
			if(winner2 != null)
			{
				winner2.addItem(ProcessType.EVENT, 13067, 5, winner2, true);
			}
		}
		...
		private void rewardAsLooser(boolean isRed)
		{
			...
			for(Entry<L2PcInstance, Integer> entry : tempPoints.entrySet())
			{
				...
				{
					player.addItem(ProcessType.EVENT, 13067, 2, player, true);
				}
			}
		}
		...
		{
			ExBlockUpSetState end = new ExBlockUpSetState(_isRedWinner);
			...
			for(L2PcInstance player : _holder.getAllPlayers())
			{
				...
				player.setTeam(0);
				// Set default arena
				player.getEventController().setHandysBlockCheckerEventArena(DEFAULT_ARENA);
