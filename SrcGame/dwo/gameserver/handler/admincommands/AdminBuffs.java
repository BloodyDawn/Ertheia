...
public class AdminBuffs implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if(command.startsWith("admin_areacancel"))
		{
			...
			try
			{
				...
				activeChar.getKnownList().getKnownCharactersInRadius(radius).stream().filter(knownChar -> knownChar instanceof L2PcInstance && !knownChar.equals(activeChar)).forEach(L2Character::stopAllEffects);
		...
		else if(command.startsWith("admin_removereuse"))
		{
			...
			try
			{
				...
				activeChar.sendMessage("Откат умений удален игроку " + player.getName() + '.');
				return true;
			}
		...
		else if(command.startsWith("admin_switch_gm_buffs"))
		{
			...
			{
				activeChar.sendMessage("Функция не доступна.");
				return false;
			}
			else
			{
				boolean toAuraSkills = activeChar.getKnownSkill(7041) != null;
				SkillTreesData.getInstance().removeGmSkills(activeChar);
				if(toAuraSkills)
				{
					SkillTreesData.getInstance().addGmSkills(activeChar, toAuraSkills);
				}
				activeChar.sendSkillList();
				activeChar.sendMessage("Тип цели для ГМ скилов изменен на: " + (toAuraSkills ? "массовый" : "одиночный"));
				return true;
			}
		}
		else
	...
	private void removeBuff(L2PcInstance activeChar, int objId, int skillId)
	{
		...
		try
		{
			target = (L2Character) WorldManager.getInstance().findObject(objId);
		}
		catch(Exception e)
		{
		}
