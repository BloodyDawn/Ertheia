...
public class AdminInstance implements IAdminCommandHandler
{
	private static final String[] ADMIN_COMMANDS = {
		"admin_setinstance", "admin_ghoston", "admin_ghostoff", "admin_createinstance", "admin_destroyinstance",
		"admin_listinstances", "admin_setinstancetime"
	};
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		if(command.startsWith("admin_createinstance"))
		{
			...
			if(parts.length == 3)
			{
				try
				{
					...
					if(id < 300000 && InstanceManager.getInstance().createInstanceFromTemplate(id, parts[2]) && id < 300000)
					{
		...
		else if(command.startsWith("admin_setinstancetime"))
		{
			try
			{
				...
				long timeInMillis = Calendar.getInstance().getTimeInMillis() + Integer.parseInt(st.nextToken()) * 60 * 1000;
				InstanceManager.getInstance().setInstanceTime(target.getObjectId(), instance, timeInMillis);
				Calendar cal = Calendar.getInstance();
				cal.setTimeInMillis(timeInMillis);
				activeChar.sendMessage("Время отката иста " + InstanceManager.getInstance().getInstanceIdName(instance) + " для игрока " + target.getName() + " установлино на " + cal.getTime());
			}
			catch(Exception e)
			{
				activeChar.sendMessage("Use //setinstancetime id time[minute]");
			}
		}
		else if(command.startsWith("admin_listinstances"))
		{
			Map<Integer, Instance> instanceMap = InstanceManager.getInstance().getInstances();
			for(Instance temp : instanceMap.values())
			{
		...
		else if(command.startsWith("admin_setinstance"))
		{
			try
			{
				...
				else
				{
					...
					target.getInstanceController().setInstanceId(val);
					if(target instanceof L2PcInstance)
					{
						...
						if(!player.getPets().isEmpty())
						{
							...
							{
								pet.getInstanceController().setInstanceId(val);
