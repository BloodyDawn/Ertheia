...
public class AdminEventEngine implements IAdminCommandHandler
{
	private static final String[] ADMIN_COMMANDS = {
		"admin_event", "admin_event_new", "admin_event_choose", "admin_event_store", "admin_event_set",
		"admin_event_change_teams_number", "admin_event_announce", "admin_event_panel", "admin_event_control_begin",
		"admin_event_control_teleport", "admin_add", "admin_event_see", "admin_event_del", "admin_delete_buffer",
		"admin_event_control_sit", "admin_event_name", "admin_event_control_kill", "admin_event_control_res",
		"admin_event_control_poly", "admin_event_control_unpoly", "admin_event_control_prize",
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if(command.startsWith("admin_event_see"))
		{
			...
			try
			{
				...
				String replyMSG = StringUtil.concat("<html><body>" + "<center><font color=\"LEVEL\">", eventName, "</font><font color=\"FF0000\"> bY ", inbr.readLine(), "</font></center><br>" + "<br>", inbr.readLine(), "</body></html>");
		...
		else if(command.startsWith("admin_event_announce"))
		{
			...
			String replyMSG = StringUtil.concat("<html><body>" +
	...
	private String showStoredEvents()
	{
		...
		StringBuilder result = new StringBuilder(files.length * 500);
		result.append("<table>");
	...
	public void showMainPage(L2PcInstance activeChar)
	{
		...
		String replyMSG = StringUtil.concat("<html><body>" +
			...
			"<center><br>Stored Events<br></center>", showStoredEvents(), "</body></html>");
		adminReply.setHtml(replyMSG);
		activeChar.sendPacket(adminReply);
	}
	...
	public void showNewEventPage(L2PcInstance activeChar)
	{
		...
		StringBuilder replyMSG = StringUtil.startAppend(500, "<html><body>" +
			"<center><font color=\"LEVEL\">[ L2J EVENT ENGINE ]</font></center><br>" +
	...
	public void showEventParameters(L2PcInstance activeChar, int teamnumbers)
	{
		...
		StringBuilder replyMSG = StringUtil.startAppend(1000 + teamnumbers * 150, "<html><body>" +
			"<center><font color=\"LEVEL\">[ L2J EVENT ENGINE ]</font></center><br>" +
			"<center><font color=\"LEVEL\">", L2Event.eventName, "</font></center><br>" +
	...
	{
		L2Event.npcs.clear();
		Deque<L2PcInstance> temp = new LinkedList<>();
		temp.clear();
		for(L2PcInstance player : WorldManager.getInstance().getAllPlayersArray())
		{
			...
			temp.addAll(player.getKnownList().getKnownPlayers().values().stream().filter(playertemp -> Math.abs(playertemp.getX() - player.getX()) < 500 && Math.abs(playertemp.getY() - player.getY()) < 500 && Math.abs(playertemp.getZ() - player.getZ()) < 500).collect(Collectors.toList()));
		}
	...
	void showEventControl(L2PcInstance activeChar)
	{
		...
		StringBuilder replyMSG = StringUtil.startAppend(1000, "<html><body>" + "<center><font color=\"LEVEL\">[ L2J EVENT ENGINE ]</font></center><br><font color=\"LEVEL\">", L2Event.eventName, "</font><br><br><table width=200>" +
	...
	String getMaxLeveledPlayer()
	{
		...
		for(String tempName2 : L2Event.participatingPlayers)
		{
			try
			{
				pc = WorldManager.getInstance().getPlayer(tempName2);
				if(max < pc.getLevel())
				{
					max = pc.getLevel();
					name = tempName2;
	...
	void destroyEventNpcs()
	{
		...
		while(!L2Event.npcs.isEmpty())
		{
			try
			{
				npc = (L2Npc) WorldManager.getInstance().findObject(Integer.parseInt(L2Event.npcs.getFirst()));
	...
	{
		Iterable<String> linked = L2Event.players.get(team);
		for(String aLinked : linked)
		{
			try
			{
				L2PcInstance pc = WorldManager.getInstance().getPlayer(aLinked);
	...
	void sitTeam(int team)
	{
		...
		for(String aLinked : linked)
		{
			...
			{
				L2PcInstance pc = WorldManager.getInstance().getPlayer(aLinked);
				pc.getEventController().setForceSit(!pc.getEventController().isForceSit());
				if(pc.getEventController().isForceSit())
				{
					pc.sitDown();
				}
				else
	...
	void killTeam(L2PcInstance activeChar, int team)
	{
		...
		for(String aLinked : linked)
		{
			try
			{
				L2PcInstance target = WorldManager.getInstance().getPlayer(aLinked);
	...
	void resTeam(int team)
	{
		...
		for(String aLinked : linked)
		{
			L2PcInstance character = WorldManager.getInstance().getPlayer(aLinked);
	...
	void polyTeam(int team, String id)
	{
		...
		for(String aLinked : linked)
		{
			...
			{
				L2PcInstance target = WorldManager.getInstance().getPlayer(aLinked);
				target.getPolyController().setInfo(PolyController.PolyType.NPC, Integer.parseInt(id));
				target.teleToLocation(target.getX(), target.getY(), target.getZ(), true);
				CI info1 = new CI(target);
	...
	{
		player.getInventory().addItem(ProcessType.EVENT, id, num, player, activeChar);
		ItemList il = new ItemList(player, true);
		player.sendPacket(il);

		NpcHtmlMessage adminReply = new NpcHtmlMessage(5);
		adminReply.setHtml("<html><body>" +
			"ПОЗДРАВЛЯЮ, Ваш подарок в Вашем инвентаре." +
			"</body></html>");
	...
	void regardTeam(L2PcInstance activeChar, int team, int n, int id, String type)
	{
		...
		for(String aLinked : linked)
		{
			try
			{
				...
				{
					temp = type.equalsIgnoreCase("kills") ? n * target.getEventController().getKillsCount() : n;
				}
	...
	void telePlayersBack(int team)
	{
		...
		for(String aLinked : linked)
		{
			...
			{
				L2PcInstance target = WorldManager.getInstance().getPlayer(aLinked);
				Location loc = target.getLocationController().getMemorizedLocation();

				target.setTitle(target.getEventController().getTitle());
				target.setReputation(target.getEventController().getReputation());
				target.setPvpKills(target.getEventController().getPvPKills());
				target.setPkKills(target.getEventController().getPkKills());
				target.teleToLocation(loc.getX(), loc.getY(), loc.getZ(), true);
				target.getEventController().clearKillsList();
				target.getEventController().setForceSit(false);
				target.getEventController().setParticipating(false);
			}
			catch(Exception ignored)
			{
			}
