...
public class AdminCreateItem implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if(command.startsWith("admin_give_item_to_all"))
		{
			...
			for(L2PcInstance onlinePlayer : WorldManager.getInstance().getAllPlayersArray())
			{
				if(!activeChar.equals(onlinePlayer) && onlinePlayer.isOnline() && onlinePlayer.getClient() != null && !onlinePlayer.getClient().isDetached())
				{
					onlinePlayer.getInventory().addItem(ProcessType.ADMIN, idval, numval, onlinePlayer, activeChar);
	...
	private void createItem(L2PcInstance activeChar, L2PcInstance target, int id, long num)
	{
		...
		target.getInventory().addItem(ProcessType.ADMIN, id, num, activeChar, null);
	...
	private int getCoinId(String name)
	{
		...
		{
			id = name.equalsIgnoreCase("fantasyislecoin") ? 13067 : 0;
		}
