...
public class AdminExpSp implements IAdminCommandHandler
{
	...
	private static Logger _log = LogManager.getLogger(AdminExpSp.class);
	...
	private void addExpSp(L2PcInstance activeChar)
	{
		...
		adminReply.setFile(activeChar.getLang(), "mods/admin/expsp.htm");
	...
	private boolean adminAddExpSp(L2PcInstance activeChar, String ExpSp)
	{
		...
		if(expval != 0 || spval != 0)
		{
			...
			if(Config.DEBUG)
			{
				_log.log(Level.DEBUG, "GM: " + activeChar.getName() + '(' + activeChar.getObjectId() + ") added " + expval + " xp and " + spval + " sp to " + player.getObjectId() + '.');
			}
		}
		return true;
	}
	...
	private boolean adminRemoveExpSP(L2PcInstance activeChar, String ExpSp)
	{
		...
		if(expval != 0 || spval != 0)
		{
			...
			if(Config.DEBUG)
			{
				_log.log(Level.INFO, "GM: " + activeChar.getName() + '(' + activeChar.getObjectId() + ") removed " + expval + " xp and " + spval + " sp from " + player.getObjectId() + '.');
			}
		}
		return true;
	}
