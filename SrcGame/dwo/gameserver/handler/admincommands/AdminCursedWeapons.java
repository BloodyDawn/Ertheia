	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		if(command.startsWith("admin_cw_info"))
		{
			if(command.contains("menu"))
			{
				...
				adminReply.setFile(activeChar.getLang(), "mods/admin/cwinfo.htm");
			...
			else
			{
				...
				for(CursedWeapon cw : cwm.getCursedWeapons())
				{
					...
					if(cw.isActivated())
					{
						...
						activeChar.sendMessage("    Player karma: " + cw.getPlayerReputation());
		...
		else
		{
			...
			else if(command.startsWith("admin_cw_add"))
			{
				...
				else
				{
					...
					{
						((L2PcInstance) target).addItem(ProcessType.ADMIN, id, 1, target, true);
					}
					else
					{
						activeChar.addItem(ProcessType.ADMIN, id, 1, activeChar, true);
					}
