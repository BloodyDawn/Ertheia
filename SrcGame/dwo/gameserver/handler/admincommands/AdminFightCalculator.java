...
public class AdminFightCalculator implements IAdminCommandHandler
{
	...
	private void handleStart(String params, L2PcInstance activeChar)
	{
		...
		{
			replyMSG = StringUtil.concat("<html><title>Selected mobs to fight</title>" +
				"<body>" +
				"<table>" +
				"<tr><td>First</td><td>Second</td></tr>" +
				"<tr><td>level ", String.valueOf(lvl1), "</td><td>level ", String.valueOf(lvl2), "</td></tr>" + "<tr><td>id ", String.valueOf(npc1.getNpcId()), "</td><td>id ", String.valueOf(npc2.getNpcId()), "</td></tr>" + "<tr><td>", String.valueOf(npc1.getName()), "</td><td>", npc2.getName(), "</td></tr>" +
				"</table>" +
				"<center><br><br><br>" +
				"<button value=\"OK\" action=\"bypass -h admin_fight_calculator_show ", String.valueOf(npc1.getNpcId()), " ", String.valueOf(npc2.getNpcId()), "\"  width=100 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" +
				"</center>" +
				"</body></html>");
		}
		else if(lvl1 != 0 && npc1 == null)
		{
			List<L2NpcTemplate> npcs = NpcTable.getInstance().getAllOfLevel(lvl1);
		...
		{
			List<L2NpcTemplate> npcs = NpcTable.getInstance().getAllOfLevel(lvl2);
			StringBuilder sb = StringUtil.startAppend(50 + npcs.size() * 200, "<html><title>Select second mob to fight</title>" + "<body><table>");

			for(L2NpcTemplate n : npcs)
			{
				StringUtil.append(sb, "<tr><td><a action=\"bypass -h admin_fight_calculator lvl1 ", String.valueOf(lvl1), " lvl2 ", String.valueOf(lvl2), " mid1 ", String.valueOf(mid1), " mid2 ", String.valueOf(n.getNpcId()), "\">", String.valueOf(n.getName()), "</a></td></tr>");
			}
		...
		{
			replyMSG = "<html><title>Select mobs to fight</title>" +
				"<body>" +
				"<table>" +
				"<tr><td>First</td><td>Second</td></tr>" +
	...
	private void handleShow(String params, L2PcInstance activeChar)
	{
		...
		{
			boolean _miss1 = AttackSpeedAndMiss.calcHitMiss(npc1, npc2);
			if(_miss1)
			{
				miss1++;
			}
			byte _shld1 = Shield.calcShldUse(npc1, npc2, null, false);
			if(_shld1 > 0)
			{
				shld1++;
			}
			boolean _crit1 = PhysicalDamage.calcCrit(npc1.getCriticalHit(npc2, null), false, npc2);
			if(_crit1)
			{
				crit1++;
			}
			...
			if(!_miss1)
			{
				...
				double _dmg1 = PhysicalDamage.calcPhysDam(npc1, npc2, null, _shld1, _crit1, false, false);
				dmg1 += _dmg1;
				npc1.abortAttack();
			}
		}
		...
		for(int i = 0; i < 10000; i++)
		{
			boolean _miss2 = AttackSpeedAndMiss.calcHitMiss(npc2, npc1);
			if(_miss2)
			{
				miss2++;
			}
			byte _shld2 = Shield.calcShldUse(npc2, npc1, null, false);
			if(_shld2 > 0)
			{
				shld2++;
			}
			boolean _crit2 = PhysicalDamage.calcCrit(npc2.getCriticalHit(npc1, null), false, npc1);
			if(_crit2)
			{
				crit2++;
			}
			...
			if(!_miss2)
			{
				...
				double _dmg2 = PhysicalDamage.calcPhysDam(npc2, npc1, null, _shld2, _crit2, false, false);
				dmg2 += _dmg2;
				npc2.abortAttack();
			}
		}
		...
		int hp1 = (int) (HpMpCpRegen.calcHpRegen(npc1) * 100000 / npc1.getRegeneratePeriod());

		double maxHp2 = npc2.getMaxHp();
		int hp2 = (int) (HpMpCpRegen.calcHpRegen(npc2) * 100000 / npc1.getRegeneratePeriod());

		NpcHtmlMessage adminReply = new NpcHtmlMessage(5);

		StringBuilder replyMSG = StringUtil.startAppend(1000, "<html><title>Selected mobs to fight</title>" +
			"<body>" +
			"<table>");
		...
		if(!params.isEmpty())
		{
			npc1.getLocationController().delete();
