...
public class KnockDown extends L2Effect
{
	private int _x;
	private int _y;
	private int _z;

	public KnockDown(Env env, EffectTemplate template)
	{
		super(env, template);
	}
	...
	{
		return L2EffectType.KNOCK_DOWN;
	}
	...
	public boolean onStart()
	{
		...
		if(distance > 2000)
		{
			_log.log(Level.INFO, "Effect KnockDown was going to use invalid coordinates for characters, getEffected: " + curX + ',' + curY + " and getEffector: " + getEffector().getX() + ',' + getEffector().getY());
			return false;
		}
		...
		getEffected().broadcastPacket(new FlyToLocation(getEffected(), _x, _y, _z, FlyType.PUSH_DOWN_HORIZONTAL, getSkill().getFlySpeed(), getSkill().getFlyDelay(), getSkill().getFlyAnimationSpeed()));
	...
	public void onExit()
	{
		getEffected().stopKnockDown(false);
	}
