 ...
 * @author GKR
 */
public class HB_DemonPrinceFloor extends Quest
{
	private static final String qn = "HB_DemonPrinceFloor";
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if(npc.getNpcId() == GK_4)
		{
			...
			if(htmltext == null)
			{
				enterInstance(player, "HB_DemonPrinceFloor.xml");
			}
		}
	...
	private boolean checkTeleport(L2PcInstance player)
	{
		...
		for(L2PcInstance partyMember : party.getMembers())
		{
			...
			Long reentertime = InstanceManager.getInstance().getInstanceTime(partyMember.getObjectId(), InstanceZoneId.TOWER_OF_INFINITUM_5TH_FLOOR.getId());
	...
	private int enterInstance(L2PcInstance player, String template)
	{
		...
		else
		{
			...
			world.templateId = InstanceZoneId.TOWER_OF_INFINITUM_5TH_FLOOR.getId();
			world.status = 0;
			InstanceManager.getInstance().addWorld(world);
			_log.log(Level.INFO, "Tower of Infinitum - Demon Prince floor started " + template + " Instance: " + instanceId + " created by player: " + player.getName());

			for(L2PcInstance partyMember : player.getParty().getMembers())
			{
				partyMember.teleToInstance(ENTRY_POINT, instanceId);
				partyMember.destroyItemByItemId(ProcessType.QUEST, SEAL_BREAKER_5, 1, null, true);
