...
public class CrystalCaverns extends Quest
{
	...
	private static final int[] MOBLIST = {
		22279, 22280, 22281, 22282, 22283, 22285, 22286, 22287, 22288, 22289, 22293, 22294, 22295, 22296, 22297, 22305,
		22306, 22307, 22416, 22418, 22419, 22420
	};
	// Doors/Walls/Zones
	...
	private static final int[][] ordreOracle1 = {
		{32274, 147090, 152505, -12169, 31613}, {32275, 147090, 152575, -12169, 31613},
		{32274, 147090, 152645, -12169, 31613}, {32274, 147090, 152715, -12169, 31613}
	};
	private static final int[][] ordreOracle2 = {
		{32274, 149783, 152505, -12169, 31613},
		// {32274, 149783, 152575, -12169, 31613},
		{32274, 149783, 152645, -12169, 31613}, {32276, 149783, 152715, -12169, 31613}
	};
	...
	private static int[][] SPAWNS = {
		{141842, 152556, -11814, 50449}, {141503, 153395, -11814, 40738}, {141070, 153201, -11814, 39292},
		{141371, 152986, -11814, 35575}, {141602, 154188, -11814, 24575}, {141382, 154719, -11814, 37640},
		{141376, 154359, -11814, 12054}, {140895, 154383, -11814, 37508}, {140972, 154740, -11814, 52690},
		{141045, 154504, -11814, 50674}, {140757, 152740, -11814, 39463}, {140406, 152376, -11814, 16599},
		{140268, 152007, -11817, 45316}, {139996, 151485, -11814, 47403}, {140378, 151190, -11814, 58116},
		{140521, 150711, -11815, 55997}, {140816, 150215, -11814, 53682}, {141528, 149909, -11814, 22020},
		{141644, 150360, -11817, 13283}, {142048, 150695, -11815, 5929}, {141852, 151065, -11817, 27071},
		{142408, 151211, -11815, 2402}, {142481, 151762, -11815, 12876}, {141929, 152193, -11815, 27511},
		{142083, 151791, -11814, 47176}, {141435, 150402, -11814, 41798}, {140390, 151199, -11814, 50069},
		{140557, 151849, -11814, 45293}, {140964, 153445, -11814, 56672}, {142851, 154109, -11814, 24920},
		{142379, 154725, -11814, 30342}, {142816, 154712, -11814, 33193}, {142276, 154223, -11814, 33922},
		{142459, 154490, -11814, 33184}, {142819, 154372, -11814, 21318}, {141157, 154541, -11814, 27090},
		{141095, 150281, -11814, 55186}
	};
	//first spawns
	private static int[][] FIRST_SPAWNS = {
		{22276, 148109, 149601, -12132, 34490}, {22276, 148017, 149529, -12132, 33689},
	...
	private static int[][] EMERALD_SPAWNS = {
		{22280, 144437, 143395, -11969, 34248}, {22281, 149241, 143735, -12230, 24575},
	...
	private static int[][] ROOM1_SPAWNS = {
		{22288, 143114, 140027, -11888, 15025}, {22288, 142173, 140973, -11888, 55698},
	...
	private static int[][] ROOM2_SPAWNS = {
		{22303, 146276, 141483, -11880, 34643}, {22287, 145707, 142161, -11880, 28799},
	...
	private static int[][] ROOM3_SPAWNS = {
		{22302, 145123, 143713, -12808, 65323}, {22294, 145188, 143331, -12808, 496},
	...
	private static int[][] ROOM4_SPAWNS = {
		{22304, 150563, 142240, -12108, 16454}, {22294, 150769, 142495, -12108, 16870},
	...
	private static int[][] STEAM1_SPAWNS = {
		{22305, 145260, 152387, -12165, 32767}, {22305, 144967, 152390, -12165, 30464},
	...
	private static int[][] STEAM2_SPAWNS = {
		{22306, 147740, 152767, -12165, 65043}, {22306, 148215, 152828, -12165, 970},
	...
	private static int[][] STEAM3_SPAWNS = {
		{22307, 150735, 152316, -12145, 31930}, {22307, 150725, 152467, -12165, 33635},
	...
	private boolean checkConditions(L2PcInstance player)
	{
		...
		for(L2PcInstance partyMember : party.getMembers())
		{
			...
			Long reentertime = InstanceManager.getInstance().getInstanceTime(partyMember.getObjectId(), InstanceZoneId.CRYSTAL_CAVERNS.getId());
	...
	private void Throw(L2Character effector, L2Character effected)
	{
		...
		{
			Location destiny = GeoEngine.getInstance().moveCheck(effected.getX(), effected.getY(), effected.getZ(), _x, _y, _z, effected.getInstanceId());
			_x = destiny.getX();
			_y = destiny.getY();
		}
		effected.broadcastPacket(new FlyToLocation(effected, _x, _y, _z, FlyType.THROW_UP, 0, 0, 0));
	...
	protected int enterInstance(L2PcInstance player, String template, teleCoord teleto)
	{
		...
		world.templateId = InstanceZoneId.CRYSTAL_CAVERNS.getId();
		...
		if(player.getParty() == null)
		{
			...
			InstanceManager.getInstance().setInstanceTime(player.getObjectId(), InstanceZoneId.CRYSTAL_CAVERNS.getId(), System.currentTimeMillis() + INSTANCEPENALTY);
			teleportplayer(player, teleto);
		...
		else
		{
			for(L2PcInstance partyMember : party.getMembers())
			{
				...
				InstanceManager.getInstance().setInstanceTime(partyMember.getObjectId(), InstanceZoneId.CRYSTAL_CAVERNS.getId(), System.currentTimeMillis() + INSTANCEPENALTY);
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isPet, L2Skill skill)
	{
		if(npc.getNpcId() == TEARS)
		{
			...
			if(tmpworld instanceof CCWorld)
			{
				...
				else if(!world.tears.equals(npc))
				{
					return "";
				}
				else if(!world.copys.isEmpty())
				{
					...
					if(notAOE)
					{
						...
						{
							copy.getLocationController().decay();
						}
						world.copys.clear();
					}
					return "";
				...
				if(nowHp < maxHp * 0.4 && rand < 5)
				{
					...
					if(party != null)
					{
						party.getMembers().forEach(this::stopAttack);
					}
					else
					{
						stopAttack(attacker);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if(tmpworld instanceof CCWorld)
		{
			...
			else if(event.equalsIgnoreCase("EmeraldSteam"))
			{
				...
				{
					oracle.getLocationController().decay();
				}
			}
			...
			else if(event.equalsIgnoreCase("baylor_alarm"))
			{
				if(world._alarm == null)
				{
					...
					world._alarm.broadcastPacket(new Say2(world._alarm.getObjectId(), ChatType.NPC_ALL, world._alarm.getName(), "Alarm signal was switched off! All will in the danger, if we do not take measures immediately!"));
				}
			}
				...
				else
				{
					...
					if(nowHp < maxHp * 0.2 && world._raidStatus < 3 && npc.getFirstEffect(5224) == null && npc.getFirstEffect(5225) == null)
					{
						if(nowHp < maxHp * 0.15 && world._raidStatus == 2)
						{
							...
							npc.broadcastPacket(new Say2(npc.getObjectId(), ChatType.NPC_ALL, npc.getName(), "Demons King Beleth, give me power! Aaaaa!!!"));
						}
			...
			else if(event.equalsIgnoreCase("autoFood"))
			{
				...
				List<L2Object> crystals = WorldManager.getInstance().getVisibleObjects(npc, 300).stream().filter(object -> object instanceof L2ItemInstance && ((L2ItemInstance) object).getItemId() == CRYSTALFOOD).collect(Collectors.toCollection(FastList::new));
				int minDist = 300000;
				...
				if(minDist == 300000)
				{
					if(Rnd.getChance(5))
					{
						npc.broadcastPacket(new Say2(npc.getObjectId(), ChatType.NPC_ALL, npc.getName(), "ah ... I am so hungry ..."));
					}
			...
			else if(event.equalsIgnoreCase("reachFood"))
			{
				...
				if(npc.getAI().getIntention() == CtrlIntention.AI_INTENTION_ACTIVE)
				{
					WorldManager.getInstance().removeVisibleObject(cryGolem.foodItem, cryGolem.foodItem.getLocationController().getWorldRegion());
					WorldManager.getInstance().removeObject(cryGolem.foodItem);
					npc.getAI().setIntention(CtrlIntention.AI_INTENTION_IDLE, null);
		...
		if(tmpworld instanceof CCWorld)
		{
			...
			else if(world.status == 2 && world.keyKeepers.contains(npc))
			{
				...
				else if(npc.getNpcId() == GK2)
				{
					...
					if(party != null)
					{
						party.getMembers().stream().filter(partyMember -> partyMember.getInstanceId() == world.instanceId).forEach(partyMember -> {
				...
				world.keyKeepers.stream().filter(gk -> !gk.equals(npc)).forEach(gk -> gk.getLocationController().decay());
			}
			...
			else if(world.status >= 22 && world.status <= 25)
			{
				...
				{
					world.oracles.keySet().stream().filter(oracle -> world.oracles.get(oracle).equals(npc)).forEach(oracle -> world.oracles.put(oracle, null));
				}
				if(checkKillProgress(0, npc, world))
				{
					world.npcList2.clear();
					...
					switch(world.status)
					{
						...
						case 25:
							...
							return null;
						default:
							_log.log(Level.WARN, "CrystalCavern-SteamCorridor: status " + world.status + " error. OracleOrder not found in " + world.instanceId);
							return null;
					}
		...
		if(tmpworld instanceof CCWorld)
		{
			...
			else if(npc.getNpcId() >= 32275 && npc.getNpcId() <= 32277 && world.OracleTriggered[npc.getNpcId() - 32275])
			{
				...
				if(doTeleport)
				{
					...
					else if(player.getParty() != null)
					{
						...
						{
							partyMember.destroyItemByItemId(ProcessType.QUEST, RED_CORAL, 1, player, true);
							teleportplayer(partyMember, teleto);
						}
					}
					else
			...
			else if(npc.getNpcId() == ORACLE_GUIDE_3)
			{
				if(world.status < 30 && checkBaylorConditions(player))
				{
					...
					{
						//int rnd = Rnd.get(100);
						//partyMember.destroyItemByItemId(ProcessType.QUEST, (rnd < 33 ? BOSS_CRYSTAL_1:(rnd < 67 ? BOSS_CRYSTAL_2:BOSS_CRYSTAL_3)), 1, partyMember, true); Crystals are no longer beign cunsumed while entering to Baylor Lair.
						world._raiders.addAll(party.getMembers().stream().collect(Collectors.toList()));
					}
				}
				else
				{
	...
	public String onSkillSee(L2Npc npc, L2PcInstance caster, L2Skill skill, L2Object[] targets, boolean isPet)
	{
		...
		{
			InstanceWorld tmpworld = InstanceManager.getInstance().getWorld(npc.getInstanceId());
			if(tmpworld instanceof CCWorld && Rnd.getChance(15))
			{
				((CCWorld) tmpworld).oracles.keySet().stream().filter(oracle -> !oracle.equals(npc)).forEach(oracle -> oracle.getLocationController().decay());
		...
		else if(npc.isInvul() && npc.getNpcId() == TEARS && skill.getId() == 2369 && caster != null)
		{
			...
			if(tmpworld instanceof CCWorld)
			{
				...
				if(world.dragonScaleNeed == 0 && Rnd.getChance(80))
				{
				...
				if(world.status == 8)
				{
					...
					for(L2DoorInstance door : InstanceManager.getInstance().getInstance(world.instanceId).getDoors())
					{
						if(door.getDoorId() == room + 24220000)
						{
							...
							if(((L2PcInstance) character).getItemsCount(RACE_KEY) == 0)
							{
								return null;
							}
							...
							door.openMe();
							character.destroyItemByItemId(ProcessType.NPC, RACE_KEY, 1, null, true);
			...
			if(tmpworld instanceof CCWorld)
			{
				...
				if(world.status == 8)
				{
					...
					for(L2DoorInstance door : InstanceManager.getInstance().getInstance(world.instanceId).getDoors())
					{
						...
						{
							if(door.isOpened() && world.openedDoors.get(door).equals(character))
							{
								door.closeMe();
	...
	{
		int num = Math.max((int) Config.RATE_DROP_ITEMS_BY_RAID, 1);

		L2Party party = player.getParty();
		if(party != null)
		{
			party.getMembers().stream().filter(partyMember -> partyMember.getInstanceId() == instanceId).forEach(partyMember -> {
				if(!isBaylor && partyMember.getItemsCount(CONT_CRYSTAL) > 0)
				{
					partyMember.destroyItemByItemId(ProcessType.NPC, CONT_CRYSTAL, 1, null, true);
					partyMember.addItem(ProcessType.NPC, bossCry, 1, null, true);
				}
				if(Rnd.get(10) < 5)
				{
					partyMember.addItem(ProcessType.NPC, WHITE_SEED, num, null, true);
				}
				else
				{
					partyMember.addItem(ProcessType.NPC, BLACK_SEED, num, null, true);
				}
			});
		}
		else if(player.getInstanceId() == instanceId)
		{
			if(!isBaylor && player.getItemsCount(CONT_CRYSTAL) > 0)
			{
				player.destroyItemByItemId(ProcessType.NPC, CONT_CRYSTAL, 1, null, true);
				player.addItem(ProcessType.NPC, bossCry, 1, null, true);
			}
			if(Rnd.get(10) < 5)
			{
				player.addItem(ProcessType.NPC, WHITE_SEED, num, null, true);
			}
			else
			{
				player.addItem(ProcessType.NPC, BLACK_SEED, num, null, true);
			}
		}
	}
	...
	{
		public L2ItemInstance foodItem;
	...
	private class CCWorld extends InstanceWorld
	{
		...
		public L2Npc tears;
		public boolean isUsedInvulSkill;
		public long dragonScaleStart;
		public int dragonScaleNeed;
		public int cleanedRooms;
		public long endTime;
		public List<L2Npc> copys = new FastList<>();
		public Map<L2Npc, CrystalGolem> crystalGolems = new FastMap<>();
		public int correctGolems;
		public boolean[] OracleTriggered = {false, false, false};
		public int kechisHenchmanSpawn;
		...
		public TIntObjectHashMap<Map<L2Npc, Boolean>> npcList2 = new TIntObjectHashMap<>();
		...
		private int _raidStatus;
		private long _dragonClawStart;
		private int _dragonClawNeed;
		private List<L2Npc> _animationMobs = new FastList<>();
		private L2Npc _camera;
		private L2Npc _baylor;
		private L2Npc _alarm;
