...
public class HallOfSuffering extends Quest
{
	...
	private static final int[][] TUMOR_SPAWNS = {
		{-186327, 208286, -9544}, {-184429, 211155, -9544}, {-182811, 213871, -9496}, {-181039, 216633, -9528},
		{-177264, 217760, -9544}
	};
	...
	private static final int BOSS_RESSURECT_TIME = 10 * 60 * 60 * 1000; // in milisex
	...
	private boolean checkConditions(L2PcInstance player)
	{
		...
		for(L2PcInstance partyMember : party.getMembers())
		{
			...
			Long reentertime = InstanceManager.getInstance().getInstanceTime(partyMember.getObjectId(), InstanceZoneId.SEED_OF_INFINITY_HALL_OF_SUFFERING_1.getId());
	...
	private void teleportPlayer(L2PcInstance player, int[] coords, int instanceId)
	{
		...
		if(!player.getPets().isEmpty())
		{
			...
			{
				for(L2Effect e : pet.getAllEffects())
				{
					if(e == null)
					{
						continue;
	...
	protected int enterInstance(L2PcInstance player, String template, int[] coords)
	{
		...
		else
		{
			...
			world.templateId = InstanceZoneId.SEED_OF_INFINITY_HALL_OF_SUFFERING_1.getId();
			world.status = 0;
			((HSWorld) world).startTime = System.currentTimeMillis();
			((HSWorld) world).ptLeaderName = player.getName();
			InstanceManager.getInstance().addWorld(world);
			_log.log(Level.INFO, "Hall Of Suffering started " + template + " Instance: " + instanceId + " created by player: " + player.getName());
	...
	protected void bossSimpleDie(L2Npc boss)
	{
		...
		if(boss.getLocationController().getWorldRegion() != null)
		{
			boss.getLocationController().getWorldRegion().onDeath(boss);
		}
	}
	...
	{
		String htmltext = HtmCache.getInstance().getHtmQuest(player.getLang(), "instances/SeedOfInfinity/32530-10.htm");
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if(tmpworld instanceof HSWorld)
		{
			...
			{
				/*if (Util.checkIfInRange(100, world.klanikus, world.klodekus, false))
				{
				...
				startQuestTimer("isTwinSeparated", 1000, npc, null);
				*/
			}
			else if(event.equalsIgnoreCase("ressurectTwin"))
			{
				L2Skill skill = SkillTable.getInstance().getInfo(5824, 1);
				L2Npc aliveTwin = world.klanikus.equals(npc) ? world.klodekus : world.klanikus;
				npc.doRevive();
				npc.doCast(skill);
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if(npcId == TEPIOS)
		{
			...
			{
				_log.log(Level.WARN, "Hall of Suffering: " + player.getName() + '(' + player.getObjectId() + ") is try to cheat!");
				return getPtLeaderText(player, (HSWorld) world);
			}
			if(((HSWorld) world).isRewarded)
			{
				return "32530-11.htm";
			}
	...
	public String onFirstTalk(L2Npc npc, L2PcInstance player)
	{
		if(npc.getNpcId() == TEPIOS)
		{
			...
			if(world instanceof HSWorld)
			{
				...
				if(player.getParty() != null && player.getParty().getLeaderObjectId() == player.getObjectId())
				{
					return HtmCache.getInstance().getHtmQuest(player.getLang(), "instances/SeedOfInfinity/" + ((HSWorld) world).rewardHtm);
				}
				return getPtLeaderText(player, (HSWorld) world);
			}
			else
	...
	private class HSWorld extends InstanceWorld
	{
		...
		public L2Npc klodekus;
		public L2Npc klanikus;
		public boolean isBossesAttacked;
