...
public class _10344_DayOfDestinyOrcsFate extends Quest
{
	// Квестовые персонажи
	...
	{
		addStartNpc(Ладанза);
		addTalkId(Ладанза, ИнтендантАдена, ЧленАвангарда);
		addTalkId(Трупы);
	...
	{
		new _10344_DayOfDestinyOrcsFate();
	}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if(player.getLevel() < 76 || !Util.hasChildClassWithRace(player.getClassId(), Race.Orc) || player.getClassId().level() >= ClassLevel.THIRD.ordinal())
		{
			return null;
		}
	...
	public String onTalk(L2Npc npc, QuestState st)
	{
		...
		if(npc.getNpcId() == Ладанза)
		{
			switch(st.getState())
			{
				...
				case CREATED:
					if(player.getVariablesController().get("_3rd_profession_ci" + player.getClassIndex(), Boolean.class, false) || player.getLevel() < 76 || !Util.hasChildClassWithRace(player.getClassId(), Race.Orc) || player.getClassId().level() >= ClassLevel.THIRD.ordinal())
					{
						st.exitQuest(QuestType.REPEATABLE);
						return "30865-02.htm";
					}
	...
	public void onLevelIncreased(L2PcInstance player)
	{
		if(player.getLevel() >= 76 && Util.hasChildClassWithRace(player.getClassId(), Race.Orc) && player.getClassId().level() < ClassLevel.THIRD.ordinal())
		{
			...
			if(st == null)
			{
				player.sendPacket(new TutorialShowQuestionMark(104));
			}
		}
	}
	...
	{
		return player.getLevel() >= 76 && Util.hasChildClassWithRace(player.getClassId(), Race.Orc) && player.getClassId().level() < ClassLevel.THIRD.ordinal();
