...
public class _10346_DayOfDestinyKamaelsFate extends Quest
{
	// Квестовые персонажи
	...
	{
		addStartNpc(Бром);
		addTalkId(Бром, ИнтендантАдена, ЧленАвангарда);
		addTalkId(Трупы);
	...
	{
		new _10346_DayOfDestinyKamaelsFate();
	}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if(player.getLevel() < 76 || !Util.hasChildClassWithRace(player.getClassId(), Race.Kamael) || player.getClassId().level() >= ClassLevel.THIRD.ordinal())
		{
			return null;
		}
	...
	public String onTalk(L2Npc npc, QuestState st)
	{
		...
		if(npc.getNpcId() == Бром)
		{
			switch(st.getState())
			{
				...
				case CREATED:
					if(player.getVariablesController().get("_3rd_profession_ci" + player.getClassIndex(), Boolean.class, false) || player.getLevel() < 76 || !Util.hasChildClassWithRace(player.getClassId(), Race.Kamael) || player.getClassId().level() >= ClassLevel.THIRD.ordinal())
					{
	...
	public void onLevelIncreased(L2PcInstance player)
	{
		if(player.getLevel() >= 76 && Util.hasChildClassWithRace(player.getClassId(), Race.Kamael) && player.getClassId().level() < ClassLevel.THIRD.ordinal())
		{
			...
			if(st == null)
			{
				player.sendPacket(new TutorialShowQuestionMark(106));
			}
		}
	}
	...
	{
		return player.getLevel() >= 76 && Util.hasChildClassWithRace(player.getClassId(), Race.Kamael) && player.getClassId().level() < ClassLevel.THIRD.ordinal();
