...
public class Natives extends Quest
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if(npc.getNpcId() == TRAITOR)
		{
			if(event.equalsIgnoreCase("open_door"))
			{
				...
				if(qs.getQuestItemsCount(MARK_OF_BETRAYAL) >= 10)
				{
					...
					npc.broadcastPacket(new NS(npc.getObjectId(), ChatType.ALL, npc.getNpcId(), NpcStringId.ALRIGHT_NOW_LEODAS_IS_YOURS));
				...
				{
					htmltext = qs.hasQuestItems(MARK_OF_BETRAYAL) ? "32364-01.htm" : "32364-02.htm";
				}
			}
		...
		{
			npc.broadcastPacket(new NS(npc.getObjectId(), ChatType.ALL, npc.getNpcId(), NpcStringId.HUN_HUNGRY));
			npc.doDie(null);
		}
