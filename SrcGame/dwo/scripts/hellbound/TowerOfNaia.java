...
public class TowerOfNaia extends Quest
{
	...
	private static final TIntObjectHashMap<int[]> DOORS = new TIntObjectHashMap<>();
	private static final TIntObjectHashMap<Integer> ZONES = new TIntObjectHashMap<>();
	...
	private final TIntObjectHashMap<Boolean> _activeRooms = new TIntObjectHashMap<>();
	private final TIntObjectHashMap<List<L2Npc>> _spawns = new TIntObjectHashMap<>();
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isPet, L2Skill skill)
	{
		if(_lock != null && npc.getObjectId() == _lock.getObjectId())
		{
			...
			if(remaindedHpPercent <= _counter && _controller != null)
			{
				...
				_controller.broadcastPacket(new NS(_controller.getObjectId(), ChatType.ALL, _controller.getNpcId(), NpcStringId.EMERGENCY_EMERGENCY_THE_OUTER_WALL_IS_WEAKENING_RAPIDLY));
				_counter -= 10;
			}
		}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if(event.equalsIgnoreCase("despawn_total"))
		{
			...
			{
				_sporeSpawn.stream().filter(spore -> spore != null && !spore.isDead()).forEach(spore -> spore.getLocationController().delete());
				_sporeSpawn.clear();
				initSporeChallenge();
			}
			...
			else if(_challengeState == STATE_SPORE_CHALLENGE_SUCCESSFULL && _winIndex >= 0)
			{
				...
				else
				{
					...
					while(it.hasNext())
					{
						...
						{
							spore.getLocationController().delete();
							it.remove();
		...
		else if(event.equalsIgnoreCase("18492-05.htm"))
		{
			...
			{
				htmltext = null;
				_lock.getLocationController().delete();
				_lock = null;
		...
		else if(event.equalsIgnoreCase("teleport") && _lock != null)
		{
			...
			if(party != null)
			{
				...
				{
					party.getMembers().stream().filter(partyMember -> Util.checkIfInRange(2000, partyMember, npc, true)).forEach(partyMember -> partyMember.teleToLocation(-47271, 246098, -9120, true));
					_lock.getLocationController().delete();
					_lock = null;
					cancelQuestTimers("spawn_lock");
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		...
		else if(Arrays.binarySearch(TOWER_MONSTERS, npcId) >= 0)
		{
			...
			if(managerId > 0 && _spawns.containsKey(managerId))
			{
				...
				if(spawned.isEmpty() && DOORS.containsKey(managerId))
				{
					...
					DoorGeoEngine.getInstance().getDoor(doorList[1]).openMe();
		...
		else if(npcId == MUTATED_ELPY)
		{
			...
			markElpyRespawn();
			DoorGeoEngine.getInstance().getDoor(18250025).closeMe();
		...
		else if(npcId >= SPORE_FIRE && npcId <= SPORE_EARTH && (_challengeState == STATE_SPORE_CHALLENGE_IN_PROGRESS || _challengeState == STATE_SPORE_CHALLENGE_SUCCESSFULL))
		{
			...
			if(_challengeState == STATE_SPORE_CHALLENGE_IN_PROGRESS)
			{
				...
				if(sporeGroup >= 0)
				{
					...
					if(Math.abs(_indexCount[sporeGroup]) < ELEMENT_INDEX_LIMIT && Math.abs(_indexCount[sporeGroup]) > 0 && _indexCount[sporeGroup] % 20 == 0 && Rnd.getChance(50))
					{
						String el = ELEMENTS_NAME[Arrays.binarySearch(ELEMENTS, npcId)];
						_sporeSpawn.stream().filter(spore -> spore != null && !spore.isDead() && spore.getNpcId() == npcId).forEach(spore -> {
							NS ns = new NS(spore, ChatType.ALL, SPORES_NPCSTRING_ID[Rnd.get(4)]);
					...
					// index value was reached
					{
						...
						_sporeSpawn.stream().filter(spore -> spore != null && !spore.isDead()).forEach(spore -> moveTo(spore, coord));
	...
	public String onFirstTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if(npcId >= ROOM_MANAGER_FIRST && npcId <= ROOM_MANAGER_LAST)
		{
	...
	public String onSpawn(L2Npc npc)
	{
		...
		{
			DoorGeoEngine.getInstance().getDoor(18250025).openMe();
	...
	private void initRoom(int managerId)
	{
		...
		{
			int[] doorList = DOORS.get(managerId);
			DoorGeoEngine.getInstance().getDoor(doorList[0]).openMe();
			DoorGeoEngine.getInstance().getDoor(doorList[1]).closeMe();
		}

		if(_spawns.containsKey(managerId) && _spawns.get(managerId) != null)
		{
			_spawns.get(managerId).stream().filter(npc -> npc != null && !npc.isDead()).forEach(npc -> npc.getLocationController().delete());
	...
	{
		long respawnTime = Rnd.get(43200, 216000) * 1000 + System.currentTimeMillis();
	...
	private void spawnElpy()
	{
		...
		long respawnTime = tmp == null ? 0 : Long.parseLong(tmp);
	...
	private void startRoom(int managerId)
	{
		...
		{
			int[] doorList = DOORS.get(managerId);
			DoorGeoEngine.getInstance().getDoor(doorList[0]).closeMe();
		}
	...
	private void removeForeigners(int managerId, L2Party party)
	{
		if(party != null && ZONES.containsKey(managerId) && ZoneManager.getInstance().getZoneById(ZONES.get(managerId)) != null)
		{
			...
			zone.getCharactersInside().stream().filter(ch -> ch instanceof L2PcInstance).forEach(ch -> {
				L2Party charParty = ch.getParty();
	...
	private void removeAllPlayers(int managerId)
	{
		...
		{
			L2ZoneType zone = ZoneManager.getInstance().getZoneById(ZONES.get(managerId));
			zone.getCharactersInside().stream().filter(ch -> ch instanceof L2PcInstance).forEach(ch -> ch.teleToLocation(16110, 243841, 11616));
		}
	}
