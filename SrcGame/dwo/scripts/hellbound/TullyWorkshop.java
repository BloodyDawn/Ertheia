...
import dwo.gameserver.model.world.zone.type.L2DamageZone;
...
public class TullyWorkshop extends Quest
{
	...
	private static final TIntObjectHashMap<int[]> TULLY_DOORLIST = new TIntObjectHashMap<>();
	private static final TIntObjectHashMap<int[][]> TELE_COORDS = new TIntObjectHashMap<>();
	...
	private static List<L2Npc> postMortemSpawn = new FastList<>();
	...
	private int nextServantIdx;
	private int killedFollowersCount;
	private boolean allowServantSpawn = true;
	private boolean allowAgentSpawn = true;
	private boolean allowAgentSpawn_7th = true;
	private boolean is7thFloorAttackBegan;
	private ScheduledFuture<?> _countdown;
	private L2Npc spawnedAgent;
	private L2Spawn pillarSpawn;
	...
	public TullyWorkshop()
	{
		...
		for(int npcId : TULLY_DOORLIST.keys())
		{
			if(npcId != 99999)
			{
		...
		for(int npcId : TELE_COORDS.keys())
		{
			addStartNpc(npcId);
			addTalkId(npcId);
		}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			L2DamageZone dmgZone = (L2DamageZone) ZoneManager.getInstance().getZoneById(200011);
			if(dmgZone != null)
			{
		...
		{
			L2Npc spawnedNpc = addSpawn(CUBE_68, 12527, 279714, -11622, 16384, false, 0, false);
		...
		if(event.equalsIgnoreCase("close") && TULLY_DOORLIST.containsKey(npcId))
		{
			...
			int[] doors = TULLY_DOORLIST.get(npcId);
		...
		{
			npc.broadcastPacket(new NS(npc.getObjectId(), ChatType.SHOUT, npc.getNpcId(), NpcStringId.DE_ACTIVATE_THE_ALARM));
		...
		{
			npc.getKnownList().getKnownPlayersInRadius(300).stream().filter(pl -> pl != null).forEach(pl -> pl.teleToLocation(-12176, 279696, -10492, true));
		...
		{
			npc.getKnownList().getKnownPlayersInRadius(500).stream().filter(pl -> pl != null).forEach(pl -> pl.teleToLocation(-12176, 279696, -10492, true));
		...
		else if((event.equalsIgnoreCase("up") || event.equalsIgnoreCase("down")) && TELE_COORDS.containsKey(npcId))
		{
			...
			else
			{
				...
				party.getMembers().stream().filter(partyMember -> Util.checkIfInRange(4000, partyMember, npc, true)).forEach(partyMember -> partyMember.teleToLocation(tele[0], tele[1], tele[2], true));
			}
			htmltext = null;
		}
		else if(npcId == INGENIOUS_CONTRAPTION)
		{
			...
			else if(event.equalsIgnoreCase("take_reward"))
			{
				...
				if(!alreadyHaveItem && !rewardedContraptions.contains(npc.getObjectId()))
				{
					...
					if(idx > -1 && idx < 5)
					{
						player.addItem(ProcessType.QUEST, REWARDS[idx], 1, npc, true);
		...
		else if(npcId == AGENT)
		{
			if(event.equalsIgnoreCase("tele_to_7th_floor") && !allowAgentSpawn)
			{
				...
				if(party == null)
				{
					...
					if(allowAgentSpawn_7th)
					{
						...
						{
							spawnedAgent.getLocationController().delete();
						}
				...
				else
				{
					...
					{
						party.getMembers().stream().filter(partyMember -> Util.checkIfInRange(6000, partyMember, npc, true)).forEach(partyMember -> partyMember.teleToLocation(-12501, 281397, -11936, true));
			...
			else if(event.equalsIgnoreCase("refuse") && !allowAgentSpawn_7th)
			{
				...
				spawnedFollowers.stream().filter(monster -> monster != null && !monster.isDead()).forEach(monster -> {
					...
					L2PcInstance target = player.getParty() == null ? player : player.getParty().getMembers().get(Rnd.get(player.getParty().getMembers().size()));
		...
		else if(event.equalsIgnoreCase("teleport") && npcId == DWARVEN_GHOST)
		{
			...
			else
			{
				...
				party.getMembers().stream().filter(partyMember -> Util.checkIfInRange(6000, partyMember, npc, true)).forEach(partyMember -> partyMember.teleToLocation(-12176, 279696, -13596, true));
			}
		}
		else if(npcId == CUBE_68 && event.startsWith("cube68_tp"))
		{
			...
			if(party != null)
			{
				...
				{
					party.getMembers().stream().filter(partyMember -> Util.checkIfInRange(6000, partyMember, npc, true)).forEach(partyMember -> partyMember.teleToLocation(CUBE_68_TELEPORTS[tpId][0], CUBE_68_TELEPORTS[tpId][1], CUBE_68_TELEPORTS[tpId][2], true));
				}
			}
			else
			{
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		...
		if(npcId == TULLY && npc.isInsideRadius(-12557, 273901, -9000, 1000, false, false))
		{
			...
			DoorGeoEngine.getInstance().getDoor(19260051).openMe();
			DoorGeoEngine.getInstance().getDoor(19260052).openMe();

			countdownTime = 600000;
			_countdown = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new CountdownTask(), 60000, 10000);
			NS ns = new NS(postMortemSpawn.get(0).getObjectId(), ChatType.SHOUT, postMortemSpawn.get(0).getNpcId(), NpcStringId.DETONATOR_INITIALIZATION_TIME_S1_MINUTES_FROM_NOW);
		...
		else if(npcId == TIMETWISTER_GOLEM && _countdown != null)
		{
			if(Rnd.get(1000) >= 700)
			{
				npc.broadcastPacket(new NS(npc.getObjectId(), ChatType.ALL, npc.getNpcId(), NpcStringId.A_FATAL_ERROR_HAS_OCCURRED));
				if(countdownTime > 180000)
				{
					countdownTime = Math.max(countdownTime - 180000, 60000);
					if(postMortemSpawn != null && !postMortemSpawn.isEmpty() && postMortemSpawn.get(0) != null && postMortemSpawn.get(0).getNpcId() == INGENIOUS_CONTRAPTION)
					{
						postMortemSpawn.get(0).broadcastPacket(new NS(postMortemSpawn.get(0).getObjectId(), ChatType.SHOUT, postMortemSpawn.get(0).getNpcId(), NpcStringId.ZZZZ_CITY_INTERFERENCE_ERROR_FORWARD_EFFECT_CREATED));
					}
				}
			}
			else
			{
				npc.broadcastPacket(new NS(npc.getObjectId(), ChatType.ALL, npc.getNpcId(), NpcStringId.TIME_RIFT_DEVICE_ACTIVATION_SUCCESSFUL));
				if(countdownTime > 0 && countdownTime <= 420000)
				{
					countdownTime += 180000;
					if(postMortemSpawn != null && !postMortemSpawn.isEmpty() && postMortemSpawn.get(0) != null && postMortemSpawn.get(0).getNpcId() == INGENIOUS_CONTRAPTION)
					{
						postMortemSpawn.get(0).broadcastPacket(new NS(postMortemSpawn.get(0).getObjectId(), ChatType.SHOUT, postMortemSpawn.get(0).getNpcId(), NpcStringId.ZZZZ_CITY_INTERFERENCE_ERROR_RECURRENCE_EFFECT_CREATED));
					}
				}
			}
		}
		...
		else if(npcId >= SERVANT_FIRST && npcId <= SERVANT_LAST)
		{
			...
			if(roomData[0] >= 0 && roomData[1] >= 0 && allowAgentSpawn)
			{
				...
				if(nextServantIdx == 2)
				{
					...
					if(RaidBossSpawnManager.getInstance().getRaidBossStatusId(DARION) == RaidBossSpawnManager.StatusEnum.ALIVE)
					{
				...
				else
				{
					for(int i = 0; i < 4; i++)
					{
						deathCount[roomData[0]][i] = i == roomData[1] ? 0 : (deathCount[roomData[0]][i] + 1) * Rnd.get(3);
					}
			...
			{
				npc.broadcastPacket(new NS(npc.getObjectId(), ChatType.SHOUT, npc.getNpcId(), NpcStringId.I_FAILED_PLEASE_FORGIVE_ME_DARION));
			}
			else
			{
				NS ns = new NS(npc.getObjectId(), ChatType.SHOUT, npc.getNpcId(), NpcStringId.S1_ILL_BE_BACK_DONT_GET_COMFORTABLE);
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		if(npc.getNpcId() == TOMBSTONE)
		{
			...
			party.getMembers().stream().filter(pl -> pl != null && Util.checkIfInRange(6000, pl, npc, false)).forEach(pl -> pl.teleToLocation(26612, 248567, -2856));
		}
		return null;
	}
	...
	public String onFirstTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if(npcId == INGENIOUS_CONTRAPTION)
		{
			if(talkedContraptions.contains(npc.getObjectId()))
			{
				return "32371-02.htm";
			}
			if(!brokenContraptions.contains(npc.getObjectId()))
			{
				if(classId.equalsOrChildOf(ClassId.maestro))
				{
		...
		if(npcId == DWARVEN_GHOST)
		{
			if(postMortemSpawn.indexOf(npc) == 11)
			{
				npc.broadcastPacket(new NS(npc.getObjectId(), ChatType.ALL, npc.getNpcId(), NpcStringId.HA_HA_YOU_WERE_SO_AFRAID_OF_DEATH_LET_ME_SEE_IF_YOU_FIND_ME_IN_TIME_MAYBE_YOU_CAN_FIND_A_WAY));
				npc.getLocationController().delete();
				return null;
			}
		...
		if(npcId == CUBE_68)
		{
			...
			if(npc.isInsideRadius(-12752, 279696, -10492, 100, true, false))
			{
				return "32467-02.htm";
			}
			return "32467-03.htm";
	...
	public String onSpellFinished(L2Npc npc, L2PcInstance player, L2Skill skill)
	{
		...
		{
			NS ns = new NS(npc.getObjectId(), ChatType.ALL, npc.getNpcId(), NpcStringId.S1_THANK_YOU_FOR_GIVING_ME_YOUR_LIFE);
			ns.addStringParameter(player.getName());
			npc.broadcastPacket(ns);

			int dmg = (int) (player.getCurrentHp() / (npc.getNpcId() - 22404));
	...
	public String onSpawn(L2Npc npc)
	{
		...
		{
			postMortemSpawn.stream().filter(spawnedNpc -> spawnedNpc != null).forEach(spawnedNpc -> spawnedNpc.getLocationController().delete());
		...
		{
			npc.setIsInvul(RaidBossSpawnManager.getInstance().getRaidBossStatusId(DARION) == RaidBossSpawnManager.StatusEnum.ALIVE);
		}
	...
	private void do7thFloorDespawn()
	{
		...
		spawnedFollowers.stream().filter(monster -> monster != null && !monster.isDead()).forEach(monster -> monster.getLocationController().delete());

		spawnedFollowerMinions.stream().filter(monster -> monster != null && !monster.isDead()).forEach(monster -> monster.getLocationController().delete());
	...
	private void doOnLoadSpawn()
	{
		...
		if(RaidBossSpawnManager.getInstance().getRaidBossStatusId(TULLY) != RaidBossSpawnManager.StatusEnum.ALIVE)
		{
			for(int i = 12; i <= 13; i++)
			{
	...
	{
		DoorGeoEngine.getInstance().getDoor(20250005).openMe();
		DoorGeoEngine.getInstance().getDoor(20250004).openMe();
	...
	{
		DoorGeoEngine.getInstance().getDoor(20250009).closeMe();
		DoorGeoEngine.getInstance().getDoor(20250008).closeMe();
		ThreadPoolManager.getInstance().scheduleGeneral(new DoorTask(new int[]{
			20250777, 20250778
		}, STATE_OPEN), 1000);
		ThreadPoolManager.getInstance().scheduleGeneral(new DoorTask(new int[]{
	...
	private static class DoorTask implements Runnable
	{
		...
		public void run()
		{
			...
			{
				door = DoorGeoEngine.getInstance().getDoor(doorId);
				if(door != null)
				{
					switch(_state)
					{
		...
		public void run()
		{
			...
			if(countdownTime > 60000)
			{
				if(countdownTime % 60000 == 0)
				{
					...
					{
						NS ns = new NS(npc.getObjectId(), ChatType.SHOUT, npc.getNpcId(), NpcStringId.S1_MINUTES_REMAINING);
						ns.addStringParameter(Integer.toString(countdownTime / 60000));
						npc.broadcastPacket(ns);
					}
				}
			...
			else if(countdownTime <= 0)
			{
				...
				postMortemSpawn.stream().filter(spawnedNpc -> spawnedNpc != null && (spawnedNpc.getNpcId() == INGENIOUS_CONTRAPTION || spawnedNpc.getNpcId() == TIMETWISTER_GOLEM)).forEach(spawnedNpc -> spawnedNpc.getLocationController().delete());
			...
			else
			{
				if(npc != null && npc.getNpcId() == INGENIOUS_CONTRAPTION)
				{
					NS ns = new NS(npc.getObjectId(), ChatType.SHOUT, npc.getNpcId(), NpcStringId.S1_SECONDS_REMAINING);
