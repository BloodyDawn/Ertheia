...
public class MC_Show extends Quest
{
	...
	private static NpcStringId[] messages = {
		...
		NpcStringId.THANK_YOU_ALL_FOR_COMING_HERE_TONIGHT, NpcStringId.IT_IS_AN_HONOR_TO_HAVE_THE_SPECIAL_SHOW_TODAY,
		...
		NpcStringId.HERE_SHE_COMES, NpcStringId.THANK_YOU_VERY_MUCH_LEYLA,
		NpcStringId.JUST_BACK_FROM_THEIR_WORLD_TOUR_PUT_YOUR_HANDS_TOGETHER_FOR_THE_FANTASY_ISLE_CIRCUS,
		NpcStringId.COME_ON_EVERYONE, NpcStringId.DID_YOU_LIKE_IT_THAT_WAS_SO_AMAZING,
		NpcStringId.NOW_WE_ALSO_INVITED_INDIVIDUALS_WITH_SPECIAL_TALENTS,
		NpcStringId.LETS_WELCOME_THE_FIRST_PERSON_HERE, NpcStringId.OH,
		NpcStringId.OKAY_NOW_HERE_COMES_THE_NEXT_PERSON_COME_ON_UP_PLEASE,
		NpcStringId.OH_IT_LOOKS_LIKE_SOMETHING_GREAT_IS_GOING_TO_HAPPEN_RIGHT, NpcStringId.OH_MY,
		NpcStringId.THATS_G_GREAT_NOW_HERE_COMES_THE_LAST_PERSON, NpcStringId.NOW_THIS_IS_THE_END_OF_TODAYS_SHOW,
	...
	private void scheduleTimer()
	{
		...
		if(Config.DEBUG)
		{
			...
			_log.log(Level.DEBUG, "Fantasy Isle: MC show script starting at " + format.format(System.currentTimeMillis() + diff) + " and is scheduled each next 4 hours.");
		}
	...
	{
		npc.broadcastPacket(new NS(npc.getObjectId(), type, npc.getNpcId(), npcString));
	}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			_log.log(Level.WARN, "MC_Show: Null/Empty event for npc " + npc + " and player " + player + '!');
			return null;
		}
		...
		else if(npc != null && isStarted)
		{
			...
			{
				autoChat(npc, messages[6], ChatType.SHOUT);
			...
			else if(event.equalsIgnoreCase("7"))
			{
				...
				{
					case 32433:
						autoChat(npc, messages[7], ChatType.SHOUT);
			...
			else if(event.equalsIgnoreCase("11"))
			{
				...
				{
					case 32433:
						autoChat(npc, messages[10], ChatType.SHOUT);
						npc.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, new Location(-56698, -56430, -2008, 32768));
			...
			{
				autoChat(npc, messages[15], ChatType.SHOUT);
			...
			{
				autoChat(npc, messages[22], ChatType.ALL);
			...
			else
			{
				...
				{
					ShoutInfo si = talks.get(event);
					if(si != null)
					{
						autoChat(npc, si.getNpcStringId(), ChatType.ALL);
						startQuestTimer(si.getNextEvent(), si.getTime(), npc, null);
					}
				}
				else if(walks.containsKey(event))
				{
					WalkInfo wi = walks.get(event);
					if(wi != null)
					{
	...
	public String onSpawn(L2Npc npc)
	{
		if(isStarted)
		{
			...
			{
				case 32433:
					autoChat(npc, messages[0], ChatType.SHOUT);
