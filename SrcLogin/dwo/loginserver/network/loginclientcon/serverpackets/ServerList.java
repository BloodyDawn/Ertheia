...
public class ServerList extends L2LoginServerPacket
{
	...
	public ServerList(L2LoginClient client)
	{
		...
		_servers.addAll(GameServerTable.getInstance().getRegisteredGameServers().values().stream().map(gsi -> new ServerData(client, gsi)).collect(Collectors.toList()));
	...
	public void write()
	{
		...
		for(ServerData server : _servers)
		{
			...
			writeC(server._pvp ? 0x01 : 0x01);
			writeH(server._currentPlayers);
			writeH(server._maxPlayers);
			writeC(server._status == ServerStatus.STATUS_DOWN ? 0x00 : 0x01);
			writeD(4096); // 1: Normal, 2: Relax, 4: Public Test, 8: No Label, 16: Character Creation Restricted, 32: Event, 64: Free
		...
		if(_charsOnServers != null)
		{
			...
			for(Map.Entry<Integer, Integer> integerIntegerEntry : _charsOnServers.entrySet())
			{
				writeC(integerIntegerEntry.getKey());
				writeC(integerIntegerEntry.getValue());
				if(_charsToDelete == null || !_charsToDelete.containsKey(integerIntegerEntry.getKey()))
				{
					writeC(0x00);
				}
				else
				{
					writeC(_charsToDelete.get(integerIntegerEntry.getKey()).length);
					for(long deleteTime : _charsToDelete.get(integerIntegerEntry.getKey()))
					{
