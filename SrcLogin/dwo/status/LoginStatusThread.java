...
{
	private static final Logger _log = LogManager.getLogger(LoginStatusThread.class);
	...
	public LoginStatusThread(Socket client, int uptime, String StatusPW) throws IOException
	{
		...
		if(isValidIP(client))
		{
			...
			_print.println("Welcome To The Telnet Session.");
	...
	private boolean isValidIP(Socket client)
	{
		...
		try
		{
			...
			telnetIS = new FileInputStream(new File(Config.TELNET_CONFIG));
	...
	public void run()
	{
		...
		try
		{
			while(_usrCommand.compareTo("quit") != 0 && _usrCommand.compareTo("exit") != 0)
			{
				...
				{
                    final Map<Integer, GameServerTable.GameServerInfo> gslist = GameServerTable.getInstance().getRegisteredGameServers();
                    ...
                    else
                    {
                        ...
                        _print.println("ID\tId\tStatus\tPlayers online");

                        gslist.forEach((id, gsinfo) ->
                        {
                            _print.print(id);
                            _print.print("\t");
                            _print.print(gsinfo.getId());
                            _print.print("\t");
                            _print.print(gsinfo.getStatus());
                            _print.print("\t");
				...
				else if(_usrCommand.startsWith("unblock"))
				{
					try
					{
						...
						{
							_log.log(Level.WARN, "IP removed via TELNET by host: " + _cSocket.getInetAddress().getHostAddress());
